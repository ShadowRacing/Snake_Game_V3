...
class MultiPlayer(ctk.CTkCanvas):
    ...
    def games_focused(self, event=None):
        self.snake_canvas_1.configure(highlightthickness=self.highlightthickness, highlightbackground=self.highlightbackground)
        self.snake_canvas_2.configure(highlightthickness=self.highlightthickness, highlightbackground=self.highlightbackground)
    ...
    def game_over(self):
        ...
        self.snake_canvas_1.delete("all")
        self.snake_canvas_2.delete("all")
        self.snake_canvas_1.create_text(self.snake_canvas_1.winfo_width()/2, self.snake_canvas_1.winfo_height()/2,
                        font= FONT_LIST[16], text="GAME OVER", fill="red", tag="gameover")
        self.snake_canvas_1.create_text(self.snake_canvas_1.winfo_width()/2, self.snake_canvas_1.winfo_height()/2 + 100,
                        font= FONT_LIST[10], text="Press R to play again", fill="white", tag="gameover")
        self.snake_canvas_2.create_text(self.snake_canvas_2.winfo_width()/2, self.snake_canvas_2.winfo_height()/2,
                        font= FONT_LIST[16], text="GAME OVER", fill="red", tag="gameover")
        self.snake_canvas_2.create_text(self.snake_canvas_2.winfo_width()/2, self.snake_canvas_2.winfo_height()/2 + 100,
                        font= FONT_LIST[10], text="Press R to play again", fill="white", tag="gameover")
        ...
        self.get_time_score = int(self.config.get('Multiplayer_Snake_Values', 'high_score_time', fallback='0'))
        if self.total_time_played > self.get_time_score:
            self.config.set('Multiplayer_Snake_Values', 'high_score_time', str(self.total_time_played))
            self.logfile.log_game_event(f"high_score_time updated to: {self.total_time_played}" )

        
        self.get_snake_length = int(self.config.get('Multiplayer_Snake_Values', 'snake_length_high_score', fallback='0'))
